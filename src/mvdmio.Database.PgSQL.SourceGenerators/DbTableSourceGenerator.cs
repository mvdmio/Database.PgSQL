...
public class DbTableSourceGenerator : IIncrementalGenerator
{
   ...
   private static string GenerateSource(INamedTypeSymbol classSymbol, TableAttribute table, List<ColumnAttribute> columns)
   {
      ...
      var generatedColumnNames = string.Join(", ", columns.Where(c => c.IsGenerated).Select(c => $"\"{c.Name}\""));
      ...
      return $$"""
         ...
         {
             /// <auto-generated />
             {{accessibility}} class {{className}}DbTable : DbTable<{{className}}>
             {
                private readonly DatabaseConnection _db;
             
                protected internal override string TableName => "{{table.Name}}";
                protected internal override string Schema => "{{table.Schema}}";
                protected internal override string[] Columns => [ {{columnNames}} ];
                protected internal override string[] PrimaryKeyColumns => [ {{primaryKeyColumnNames}} ];
                protected internal override string[] GeneratedColumns => [ {{generatedColumnNames}} ];
                
                /// <summary>
                ///   Constructor.
                /// </summary>
                {{accessibility}} {{className}}DbTable(DatabaseConnection db)
                  : base(db)
                {
                  _db = db;
                }
