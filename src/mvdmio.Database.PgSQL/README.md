...
A schema file may only establish baseline rows for scopes its own assembly **vouches for**: the scopes of migrations discovered from that assembly, plus the assembly's simple name (the default scope). Header lines naming any other scope — typically a schema pulled from a database shared by several applications without `scopes` declared in `.mvdmio-migrations.yml` — are ignored for baseline recording and produce a logged warning naming the file and scope. This means an assembly whose migrations were never folded into any schema file always runs them from zero, even when another assembly's schema header claims its scope is further along. One edge cannot be vouched for: an assembly that folded *all* of its migrations into its schema **and** overrides `IDbMigration.Scope` to something other than its assembly simple name; its baseline is rejected and later migrations in that scope fail loudly (objects already exist) instead of being silently skipped. Keep the default scope, or keep at least one discovered migration carrying the overridden scope, to avoid this. See `docs/adr/0003-scope-ownership-and-baseline-vouching.md`.
