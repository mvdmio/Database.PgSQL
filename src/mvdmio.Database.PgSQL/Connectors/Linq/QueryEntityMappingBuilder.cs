   ...
   where TEntity : class
{
   ...
   ///    joined column, combined with <c>&amp;&amp;</c>.
   /// </param>
   /// <returns>The same builder, so calls can be chained.</returns>
   /// <remarks>
   ///    A predicate is checked by the compiler member by member, which is what lets a composite key — or any further
   ///    condition beyond the joined columns — be stated as ordinary C# rather than as a key type the provider cannot
   ///    check. Always an outer join: a foreign key pointing at a missing row yields nothing rather than dropping the
   ///    row that holds it, and stating it here means a consumer changing the provider's global default cannot change
   ///    what generated code means.
   /// </remarks>
   ...
   /// <returns>The same builder, so calls can be chained.</returns>
   /// <remarks>
   ///    A property typed as a concrete list matches this overload without ambiguity, so a hand-written call resolves
   ///    without stating a type argument.
   /// </remarks>
   public QueryEntityMappingBuilder<TEntity> Relation<TTarget>(
